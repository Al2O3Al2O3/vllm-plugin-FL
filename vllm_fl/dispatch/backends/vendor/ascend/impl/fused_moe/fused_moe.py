...
from .flash_common3_context import get_flash_common3_context, set_flash_common3_context
from .moe_comm_method import (
    FusedExpertsResult,
    MoECommType,
    get_moe_comm_method,
    setup_moe_comm_method,
)
from .prepare_finalize import QuantType
from .utils import (
    enable_sp,
    maybe_trans_nz,
    npu_stream_switch,
...
class AscendUnquantizedFusedMoEMethod(UnquantizedFusedMoEMethod):
    ...
    def apply(
        ...
        **kwargs,
        ...
        moe_comm_method = get_moe_comm_method(MoECommType.ALLGATHER)
...
class AscendFusedMoE(FusedMoE):
    ...
    def __init__(self, *args, **kwargs):
        ...
        ) = init_eplb_config(eplb_config, self.moe_instance_id, self.moe_config)
    ...
    def forward_impl(  # type: ignore[override]
        ...
        moe_comm_method = get_moe_comm_method(MoECommType.ALLGATHER)
        hidden_states, router_logits, mc2_mask, context_metadata = moe_comm_method.prepare(
            hidden_states=hidden_states,
            router_logits=router_logits,
        ...
        routed_out = get_moe_comm_method(MoECommType.ALLGATHER).finalize(
...
class AscendSharedFusedMoE(*(([SharedFusedMoE, AscendFusedMoE] if SharedFusedMoE else [AscendFusedMoE]))):
    ...
    def _shared_experts_part2(self, hidden_states: torch.Tensor, shared_gate_up: torch.Tensor):
        ...
        if (
            hasattr(self._shared_experts, "expert_gate")
            and self._shared_experts.expert_gate is not None
        ):
    ...
    def _validate_shared_expert_consistency(self):
        ...
        if not torch.allclose(integrated_out, split_out):
            ...
            logger.error(
                "Integrated output - sum: %s, norm: %s",
                integrated_out.sum().item(),
                integrated_out.norm().item(),
            )
            logger.error(
                "Split output - sum: %s, norm: %s",
                split_out.sum().item(),
                split_out.norm().item(),
            )
            raise ValueError(
        ...
        logger.info_once(
            "SharedFusedMoE shared experts split computation matches the integrated computation."
        )
